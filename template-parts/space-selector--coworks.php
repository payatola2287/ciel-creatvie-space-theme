<div class="spaceSelector spaceSelector--cowork">
  <div class="gfield_label">
    Are you interested in a particular coworking space?
  </div>
  <div class="gfield_description">
    <p>
      Select as many spaces as you&apos;d like. And don&apos;t worry, this isn&apos;t your final answer. Our concierge will reach out and help you pick the right choice.
    </p>
  </div>
  <script>
    function filterSpaces_coworking(checkbox) {
      var filters = [];
      var checkboxes = document.querySelectorAll('.gfield--type-spaceSelectorCowork .spaceSelector__filters input[type="checkbox"]');
      for (var i = 0; i < checkboxes.length; i++) {
        if (checkboxes[i].checked) {
          filters.push(checkboxes[i].value);
        }
      }
      var spaces = document.querySelectorAll('.gfield--type-spaceSelectorCowork .spaceSelector__space');
      ...
      var spaceSelector = document.querySelector('.gfield--type-spaceSelectorCowork');
          ...
          foreach ($tags as $tag) {
            ...
            <div class="spaceSelector__filter gchoice">
              <input
                ...
                onchange="filterSpaces_coworking(this)"
              />
  ...
  <div class="spaceSelector__spaces">
    <script>
      ...
      function updateSpaceSelectionCoworks(checkbox) {
        const spaceSelection = document.querySelector('input[name="spaceSelection--coworks"]');
        ...
        console.log(checkbox.value, ' -- ', spaceSelection.value);
      }
    </script>
    <?php
      ...
      while ($query->have_posts()) {
        global $post;
        $query->the_post();
        $terms = get_the_terms(get_the_ID(), 'featureCoworking');
        $termClasses = [];
        if ($terms) {
        ...
        <div class="spaceSelector__space <?php echo implode(' ', $termClasses) ?>">
          ...
          <div class="spaceSelector__space__button">
            <div class="gfield_checkbox">
              <div class="gchoice">
                <input type="checkbox" id="space-interest-coworks-<?php echo $post->post_name; ?>" name="spaceSelectionCheckbox--coworks" value="<?php echo $post->post_name; ?>" onchange="updateSpaceSelectionCoworks(this)" >
                <label for="space-interest-coworks-<?php echo $post->post_name; ?>">
      ...
      wp_reset_postdata();
    ?>
    <input type="hidden" name="spaceSelection--coworks" value="" />
    <!-- Hide .promo-box -->
    <style>
      .promo-box {
        display: none;
  ...
  <script>
    function showHideCoworkSpace() {
      var fieldset = document.querySelector('.__data__space-type');
      var sectionSelector = document.querySelector('.gfield--type-spaceSelectorCowork');
      var selectedRadio = fieldset.querySelector('input[type="radio"]:checked');

        if(selectedRadio && selectedRadio.value === 'Coworking Space') {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if(fieldset && sectionSelector) {
            fieldset.addEventListener('change', showHideCoworkSpace);
        }
    });
